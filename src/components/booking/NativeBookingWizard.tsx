    ...
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {booking.step === 3 && <View style={styles.section}>
          ...
          <TextInput value={districtSearch} onChangeText={setDistrictSearch} placeholder="جستجوی محله" placeholderTextColor="#94a3b8" style={[styles.input, getInputStyle('district')]} /><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, flexDirection: 'row-reverse' }}>{filteredDistricts.map((district) => <Pressable key={district} onPress={() => booking.updateAddressField('district', district)} style={[styles.chip, booking.addressDetails.district === district && styles.chipSelected]}><Text style={styles.chipText}>{district}</Text></Pressable>)}</ScrollView>
          <Text style={styles.label}>نشانی دقیق</Text><TextInput multiline value={booking.addressDetails.fullAddress} onChangeText={(value) => booking.updateAddressField('fullAddress', value)} placeholder="مثال: خیابان، کوچه، بن‌بست" placeholderTextColor="#94a3b8" style={[styles.input, styles.textArea, getInputStyle('fullAddress')]} />
          <View style={styles.choiceRow}>
            <View style={styles.labeledInput}><Text style={styles.inputLabel}>پلاک</Text><TextInput value={booking.addressDetails.plaque} onChangeText={(value) => booking.updateAddressField('plaque', value)} placeholder="مثال: ۱۲" placeholderTextColor="#94a3b8" style={[styles.input, styles.smallInput, getInputStyle('plaque')]} /></View>
            <View style={styles.labeledInput}><Text style={styles.inputLabel}>واحد</Text><TextInput value={booking.addressDetails.unit} onChangeText={(value) => booking.updateAddressField('unit', value)} placeholder="مثال: ۴" placeholderTextColor="#94a3b8" style={[styles.input, styles.smallInput, getInputStyle('unit')]} /></View>
          </View>
          <Text style={styles.label}>نام و نام خانوادگی</Text><TextInput value={booking.addressDetails.recipientName} onChangeText={(value) => booking.updateAddressField('recipientName', value)} placeholder="مثال: علی احمدی" placeholderTextColor="#94a3b8" style={[styles.input, getInputStyle('recipientName')]} /><Text style={styles.label}>شماره موبایل</Text><TextInput value={booking.addressDetails.contactPhone} onChangeText={(value) => booking.updateAddressField('contactPhone', value)} placeholder="مثال: 09121111111" placeholderTextColor="#94a3b8" keyboardType="phone-pad" style={[styles.input, getInputStyle('contactPhone')]} />
        </View>}
